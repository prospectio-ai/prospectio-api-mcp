...
class TestEnrichLeadsNodesGetLinkedInUrlsFromDuckDuckGo:
    """Tests for _get_linkedin_urls_from_duckduckgo method in EnrichLeadsNodes."""

    @pytest.fixture
    def mock_duckduckgo_client(self) -> AsyncMock:
        """Create a mock DuckDuckGoClient."""
        return AsyncMock(spec=DuckDuckGoClient)

    @pytest.fixture
    def sample_contact_info(self) -> ContactInfo:
        """Create sample ContactInfo for testing."""
        return ContactInfo(
            name="John Doe",
            email=["john.doe@acme.com"],
            title="CTO",
            phone="+1-555-1234",
            profile_url=["https://linkedin.com/in/old-profile"],
        )

    @pytest.mark.asyncio
    async def test_returns_urls_on_successful_search(
        self,
        mock_duckduckgo_client: AsyncMock,
        sample_contact_info: ContactInfo,
    ):
        """Test returns LinkedIn URLs when search succeeds."""
        mock_duckduckgo_client.search_with_fallback.return_value = LinkedInSearchResult(
            urls=["https://www.linkedin.com/in/john-doe"],
            query="test query",
            success=True,
        )
        ...
        with patch(
            "infrastructure.services.enrich_leads_agent.nodes.DuckDuckGoClient"
        ) as mock_class:
            mock_class.return_value = mock_duckduckgo_client

            from infrastructure.services.enrich_leads_agent.nodes import (
                EnrichLeadsNodes,
            )

            nodes = MagicMock(spec=EnrichLeadsNodes)
            nodes.duckduckgo_client = mock_duckduckgo_client

            result = await EnrichLeadsNodes._get_linkedin_urls_from_duckduckgo(
                nodes, sample_contact_info, "Acme Corp"
            )
    ...
    async def test_returns_empty_list_on_exception(
        ...
        """Test handles exceptions gracefully and returns empty list."""
        mock_duckduckgo_client.search_with_fallback.side_effect = Exception(
            "Unexpected error"
        )
    ...
    @pytest.mark.asyncio
    async def test_returns_empty_list_when_no_urls_found(
        self,
        mock_duckduckgo_client: AsyncMock,
        sample_contact_info: ContactInfo,
    ):
        """Test returns empty list when search succeeds but finds no URLs."""
        mock_duckduckgo_client.search_with_fallback.return_value = LinkedInSearchResult(
            urls=[],
            query="test query",
            success=True,
